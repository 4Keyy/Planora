...
export default function CompletedBranchesPage() {
  ...
  const fetchCompletedTodos = useCallback(async (page = currentPage) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to load completed branches")
    } finally {
      setLoading(false)
    }
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          ...
          <div className="inline-flex items-center gap-3 rounded-[1.5rem] border border-white/70 bg-white/80 px-4 py-3 shadow-sm">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-gray-500 mt-1">Completed branches</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        >
          ...
          <Button asChild>
            <Link href="/branches">Go to active branches</Link>
          </Button>
        </motion.div>
      ) : (
        <>
