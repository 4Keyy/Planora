                    ...
                    onJoin={async () => {
                      if (isTodoOwner(todo, user?.userId)) {
                        try {
                          ...
                          addToast({ type: "error", title: "Could not update branch" })
                        }
                      } else {
                        try {
                          ...
                          if (status === 409) {
                            addToast({ type: "warning", title: "Branch is full or you've already joined" })
                            try {
                              const fresh = await fetchTaskById(todo.id)
                              setTodos((prev) => prev.map((t) => t.id === todo.id ? { ...t, ...fresh } : t))
                            } catch { /* ignore refetch failure */ }
                          } else {
                            addToast({ type: "error", title: "Could not join branch" })
                          }
                        }
                      }
                    }}
                          ...
                          <div className="rounded-[1.75rem] border border-gray-200 bg-white/90 p-4 sm:p-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div className="space-y-1">
                              ...
                              <p className="text-sm text-gray-500 font-medium">
                                {completedTotalCount > COMPLETED_PREVIEW_SIZE
                                  ? `Open the archive to browse all ${completedTotalCount} completed branches.`
                                  : "All completed branches currently fit in this section."}
                              </p>
                            </div>
                            <Button asChild size="sm" className="rounded-xl font-bold shadow-lg shadow-black/10">
                              <Link href="/branches/completed">
                                <History className="h-4 w-4" />
                                View all completed branches
                              </Link>
                            </Button>
                          </div>
                        </>
            ...
            onLeave={async () => {
              ...
              if (isTodoOwner(todo, user?.userId)) {
                ...
                try {
                  ...
                  addToast({ type: "error", title: "Could not leave branch" })
                }
              }
            }}
          />
