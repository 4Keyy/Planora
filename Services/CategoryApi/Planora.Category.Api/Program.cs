...
using Planora.Category.Application;
...
namespace Planora.Category.Api
{
    public class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            builder.Services.AddBuildingBlocksInfrastructure(builder.Configuration, "CategoryDatabase");

            // Category Infrastructure (DbContext, Repositories, Services, JWT)
            builder.Services.AddCategoryInfrastructure(builder.Configuration);

            // ✅ JWT Authentication (Critical для защищенных endpoints)
            ...
            try
            {
                ...
                using (var scope = app.Services.CreateScope())
                {
                    ...
                    var connectionString = builder.Configuration.GetConnectionString("CategoryDatabase")!;
                    ...
                    var db = provider.GetRequiredService<CategoryDbContext>();
            ...
            {
                Log.Fatal(ex, "❌ CategoryApi terminated unexpectedly");
                Environment.Exit(1);
            }
            finally
            {
