...
- System comments (`isSystemComment: true`) render on the activity rail with an event-typed badge — a circular marker centred on the rail line carrying an icon + colour that conveys the event (created = Sparkles/violet, started working = Zap/indigo, left = LogOut/red, completed = CheckCircle2/emerald) — followed by the sentence; no edit/delete. The rail itself is a single continuous gradient line that spans the whole timeline (it lives in a content-height wrapper, so it never breaks when the feed grows and scrolls); user-message avatars are centred on the same line.
