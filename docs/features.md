...
- Motion is handled through `framer-motion` and stays GPU-only (transform/opacity): the page fades in on enter, each section reveals with a fade-and-lift the first time it scrolls into view (`whileInView`, once), the active rail item is tracked by a spring-animated sliding pill via a shared `layoutId`, and friend cards lift subtly on hover. Every animation is suppressed under `useReducedMotion()` (including switching smooth scroll to instant and collapsing the rail pill transition to zero duration). All existing Auth API calls are preserved unchanged for profile update, password/email changes, email verification, 2FA setup, session revocation, friend requests, and admin user lookup.
