...
| The keyboard block runs on the **real data layer** | `api.defaults.adapter` is swapped for an in-memory implementation of the product's own HTTP contract, so the request interceptor (auth header, CSRF echo, `traceparent`), the response interceptor (401-refresh-retry, 403-CSRF-retry, cancellation) and `parseApiResponse` all run unchanged. Only the network hop is replaced |
| The sandbox is disclosed on screen | It seeds a session and answers its own API; saying so is what separates a demo from a deception |
