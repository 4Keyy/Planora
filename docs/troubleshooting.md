...
| The app times out **on this same PC** (localhost or the LAN IP) while a VPN/proxy is connected | a VPN/proxy client on the host (sing-box / xray / Clash / Happ TUN, or a system HTTP proxy with `ProxyEnable=1` and a `ProxyOverride` not covering local/LAN) routes the host's own loopback and LAN IP into the tunnel and blackholes them — the server is healthy | run `.\Start-Planora-Local.ps1 -FixProxy` (combine with `-Lan`) — it adds `<local>`, `127.*`, `10.*`, `172.16-31.*`, `192.168.*` and the LAN IP to the WinINET proxy bypass and refreshes the browser, so **this PC's** local/LAN traffic goes direct; refresh after. **Scope:** `-FixProxy` only fixes *this* machine's browser — it does nothing for a *remote* device (see the next row). If your VPN rewrites the proxy on reconnect, re-run it or turn on the VPN's "allow LAN / bypass LAN" |
| Phone/tablet still cannot open `http://<laptop-lan-ip>:3000` (with or without `-FixProxy`) | (1) the firewall rule was never created — the UAC prompt was declined or missed; (2) a TUN-mode VPN on the host is capturing inbound LAN replies; (3) wrong/stale IP, Wi-Fi client isolation, or a VPN on the *phone* | First read the `-Lan` banner: if it reports the firewall CLOSED, open an elevated PowerShell (Win+X → "Terminal (Admin)"), run the `New-NetFirewallRule` line it prints, then re-run `-Lan`. If the firewall is open but it still fails, turn ON your VPN client's "allow LAN / bypass LAN" or stop the VPN while sharing — **`-FixProxy` does not help a remote device**. Always open the **exact** URL `-Lan` prints (the DHCP IP changes), and confirm both devices are on the same non-guest / non-AP-isolated Wi-Fi |
...
| `Error: Failed to start the connection: WebSocket failed to connect … proxy blocking WebSockets` | the browser's WebSocket upgrade to the SignalR hub (`/realtime/hubs/notifications`) is being intercepted by a system proxy / VPN tunnel | run `.\Start-Planora-Local.ps1 -FixProxy` so local/LAN (incl. the hub) bypasses the proxy, then refresh; live push then works. Even if the WS stays down, the bell still catches up over REST on tab focus/reload (see the row above) |
