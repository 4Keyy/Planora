...
| The app (localhost or LAN IP) times out while a VPN/proxy is connected | a VPN/proxy client on the host (e.g. sing-box / xray / Clash TUN, or a system HTTP proxy with `ProxyEnable=1` and `ProxyOverride` not covering local/LAN) routes the host's own loopback and LAN IP into the tunnel and blackholes them — the server is actually healthy | run `.\Start-Planora-Local.ps1 -FixProxy` (combine with `-Lan` for LAN sharing) — it adds `<local>`, `127.*`, `10.*`, `172.16-31.*`, `192.168.*` and the LAN IP to the WinINET proxy bypass and refreshes the browser, so local/LAN goes direct; refresh the page after. If your VPN rewrites the proxy on every reconnect, re-run `-FixProxy` or enable the VPN client's own "bypass LAN / allow local" setting once |
| Phone/tablet still cannot open `http://<laptop-lan-ip>:3000`, even with `-Lan -FixProxy` | wrong/stale IP, Wi-Fi client isolation, or a VPN on the *phone* | open the **exact** URL `-Lan` prints (the DHCP IP changes between sessions); confirm both devices are on the same non-guest / non-AP-isolated Wi-Fi; turn off any VPN on the phone. `-Lan` warns when it detects an interfering proxy/TUN on the host |
